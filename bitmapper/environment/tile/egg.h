namespace tEgg
{
    ...
    void BirdEgg(int _x, int _y, int index)
    {
        int dS  = int( (_y+1) * tCell::width + (_x  ) );
    ...
    void SnakeEgg(int _x, int _y, int index)
    {
        int dS  = int( (_y+1) * tCell::width + (_x  ) );
        int dE  = int( (_y  ) * tCell::width + (_x+1) );
        int dW  = int( (_y  ) * tCell::width + (_x-1) );
        if (tCell::matrix[dS] == tTile::AIR) { tCell::replace[index] = tTile::AIR; tCell::replace[dS] = tTile::SNAKE_EGG; }
        if (tCell::season > seasonID::SPRING && tCell::season < seasonID::SUMMER)
        {if (rand()%10000 < 5 &&
    ...
    void Update(int _x, int _y, int index, int current_cell)
    {
        switch (current_cell)
        {
            case tTile::FROG_EGG  : {  FrogEgg(_x, _y, index); } break;
            case tTile::TOAD_EGG  : {  ToadEgg(_x, _y, index); } break;
            case tTile::BIRD_EGG  : {  BirdEgg(_x, _y, index); } break;
            case tTile::FISH_EGG  : {  FishEgg(_x, _y, index); } break;
            case tTile::SNAKE_EGG : { SnakeEgg(_x, _y, index); } break;
        }
    }
